...
所有通知类型都可以在菜单里单独开关。**工具调用通知默认关闭**（高频事件，避免通知中心堆积）；开启后自带节流（5 秒内最多 1 条），且通知中心只保留最新一条。
...
>
> ℹ️ 自动重连：App 启动时只探测一次、**默认不自动重连**；点「▶ 启动 DSH」或「重新连接」后开启自动重连，「⏹ 停止 DSH」后暂停。
...
| 开关 | 默认 | 作用 |
| --- | --- | --- |
| 🛠️ 执行动作时通知（工具调用） | **关** | 高频事件默认关闭防堆积（通知中心仅保留最新一条）；需要时手动开启（5 秒节流） |
| ✅ 任务完成时通知 | 开 | 回合正常结束时提醒 |
